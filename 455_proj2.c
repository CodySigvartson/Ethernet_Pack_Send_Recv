...
void send_message(char if_name[], struct sockaddr_ll sk_addr, char hw_addr[], char payload[]){
	...
	strncpy(frame.ether_dhost, hw_addr, 6);
	strncpy(frame.ether_shost, if_hwaddr.ifr_hwaddr.sa_data, 6);
	...
	// include payload
	memcpy(&buff[sizeof(struct ether_header)], payload, strlen(payload)+1);
	printf("Payload: %s\n", payload);
	printf("Sending frame: [%X][%X][%X][%X][%X][%X]|[%X][%X][%X][%X][%X][%X]|[%X][%X]|%c%c\n", buff[0],buff[1],buff[2],buff[3],buff[4],buff[5],buff[6],buff[7],buff[8],buff[9],buff[10],buff[11],buff[12],buff[13],buff[14],buff[15]);

	sk_addr.sll_ifindex = if_idx.ifr_ifindex;
	sk_addr.sll_halen = ETH_ALEN;
	int byteSent = sendto(sockfd, buff, sizeof(buff), 0, (struct sockaddr*)&sk_addr, sizeof(struct sockaddr_ll));
	printf("%d bytes sent!\n", byteSent);
}
...
void recv_message(char if_name[], struct sockaddr_ll sk_addr){
	...
	unsigned char payload[BUF_SIZ-14];
	memcpy(payload, &buff[14], BUF_SIZ-14);
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1){
		if(strncmp(argv[1],"Send", 4)==0){
			printf("Preparing to send a frame!\n");
			if (argc == 5){
				mode=SEND; 
